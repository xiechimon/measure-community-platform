...
```bash
git add -A
git commit -q -m "chore: import spring-cloud-alibaba scaffold, drop demo modules"
git log --oneline -1
```
Expected: 一条提交记录。
...
```bash
git add -A
git commit -q -m "refactor: rename to com.measure / community-* naming scheme"
git log --oneline -1
```
Expected: 提交成功。
...
```bash
git add -A
git commit -q -m "feat(info): add community-info population sample module with controller test"
git log --oneline -1
```
Expected: 提交成功。
...
```bash
git add -A
git commit -q -m "feat(info): wire real MySQL datasource, DDL, Nacos config and gateway route"
git log --oneline -1
```
Expected: 提交成功。
...
cd "//wsl.localhost/Ubuntu/home/measure/ideaProject/measure-community-platform"
git add -A
git commit -q -m "docs: rewrite README for community platform + new-module guide"
git log --oneline -1
```
Expected: 提交成功。
